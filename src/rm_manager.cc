...
RC RM_Manager::CreateFile(const char *fileName, int recordSize) {
	...
	err_unpin: pfFileHandle.UnpinPage(RM_HEADER_PAGE_NUM);
	err_close: pPfm->CloseFile(pfFileHandle);
	err_destroy: pPfm->DestroyFile(fileName);
	err_return:
	// Return error
	return (rc);
}
...
RC RM_Manager::OpenFile(const char *fileName, RM_FileHandle &fileHandle) {
	...
	// Recover from inconsistent state due to unexpected error
	err_unpin: fileHandle.pfFileHandle.UnpinPage(RM_HEADER_PAGE_NUM);
	err_close: pPfm->CloseFile(fileHandle.pfFileHandle);
	err_return:
	// Return error
	return (rc);
}
