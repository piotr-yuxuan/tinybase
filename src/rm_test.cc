...
RC Test2(void) {
	...
	if ((rc = CreateFile(FILENAME, sizeof(TestRec)))
			|| (rc = OpenFile(FILENAME, fh)) || (rc = AddRecs(fh, FEW_RECS))
...
RC Test4(void) {
	...
	assert(rc == 0);
	printf("\nopening file scan\n");
	rc = fs.OpenScan(fh, INT, sizeof(int), offsetof(TestRec, num), NO_OP, &val,
			NO_HINT);
	PrintError(rc);
	...
	assert(rc == 0);
	printf("\nopening file scan\n");
	rc = fs.OpenScan(fh, INT, sizeof(int), offsetof(TestRec, num), (CompOp) 500,
			&val, NO_HINT);
	PrintError(rc);
	...
	printf("\nopening file scan\n");
	rc = fs.OpenScan(fh, INT, sizeof(int), offsetof(TestRec, num), EQ_OP, NULL,
			NO_HINT);
	PrintError(rc);
	...
	printf("\nopening file scan\n");
	rc = fs.OpenScan(fh, INT, sizeof(int) + 1, offsetof(TestRec, num), EQ_OP,
			&val, NO_HINT);
	PrintError(rc);
	assert(rc == RM_INVALIDATTR);
	printf("\nOK\n");
	...
	printf("\nopening file scan\n");
	rc = fs.OpenScan(fh, STRING, 0, offsetof(TestRec, str), EQ_OP, &val,
			NO_HINT);
	PrintError(rc);
	assert(rc == RM_INVALIDATTR);
	printf("\nOK\n");
	...
	printf("\nopening file scan\n");
	rc = fs.OpenScan(fh, INT, sizeof(int), -1, EQ_OP, &val, NO_HINT);
	PrintError(rc);
	assert(rc == RM_INVALIDATTR);
	printf("\nOK\n");
	...
	printf("\nTesting OpenScan() with invalid attribute offset...\n");
	printf("\nopening file scan\n");
	rc = fs.OpenScan(fh, FLOAT, sizeof(float), STRLEN + 500, EQ_OP, &val,
			NO_HINT);
	PrintError(rc);
	assert(rc == RM_INVALIDATTR);
	printf("\nOK\n");
