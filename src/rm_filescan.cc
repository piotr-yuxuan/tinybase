...
//
RC RM_FileScan::OpenScan(const RM_FileHandle &fileHandle, AttrType _attrType,
		int _attrLength, int _attrOffset, CompOp _compOp, void *_value,
		ClientHint _pinHint) {
...
RC RM_FileScan::GetNextRec(RM_Record &rec) {
	...
	err_unpin: pFileHandle->pfFileHandle.UnpinPage(curPageNum);
	err_return:
	// Return error
	return (rc);
}
	...
	for (; curSlotNum < pFileHandle->fileHdr.numRecordsPerPage; curSlotNum++) {
		...
		case STRING:
			cmp = memcmp(
					pData + pFileHandle->fileHdr.pageHeaderSize
							+ curSlotNum * pFileHandle->fileHdr.recordSize
							+ attrOffset, value, attrLength);
			break;
		}

		// Make decision according to comparison operator
		if ((compOp == EQ_OP && cmp == 0) || (compOp == LT_OP && cmp < 0)
				|| (compOp == GT_OP && cmp > 0) || (compOp == LE_OP && cmp <= 0)
				|| (compOp == GE_OP && cmp >= 0)
				|| (compOp == NE_OP && cmp != 0))
			break;
	}
